...
struct ReactionSelectionView: View {
    ...
    var body: some View {
                ...
                HStack(spacing: horizontalPadding) {
                    ...
                    if allowEmojiSearch, viewState.needsSearchButton {
                        ...
                        additionalEmojiPickerView()
                            .onChange(of: selectedEmoji) { selectedEmoji in
                                transitionToViewState(.picked(selectedEmoji))
                            }
                            .onChange(of: emojiEntryIsFocused) { isFocused in
                                if isFocused {
        ...
        .onChange(of: keyboardState.isShown) { isShown in
            if !isShown && viewState == .search {
